import productImg01 from "../Images/double-sofa-01.png";
import productImg02 from "../Images/double-sofa-02.png";
import productImg03 from "../Images/double-sofa-03.png";

import productImg04 from "../Images/single-sofa-01.jpg";
import productImg05 from "../Images/single-sofa-02.jpg";
import productImg06 from "../Images/single-sofa-03.jpg";
import productImg007 from "../Images/single-sofa-04.png";

import productImg07 from "../Images/arm-chair-01.jpg";
import productImg08 from "../Images/arm-chair-02.jpg";
import productImg09 from "../Images/arm-chair-03.jpg";
import productImg10 from "../Images/arm-chair-01.jpg";

import phone01 from "../Images/phone-01.jpg";
import phone02 from "../Images/phone-02.jpg";
import phone03 from "../Images/phone-03.png";
import phone04 from "../Images/phone-04.jpg";
import phone05 from "../Images/phone-05.jpg";
import phone06 from "../Images/phone-06.jpg";
import phone08 from "../Images/phone-08.png";

import watch01 from "../Images/watch-01.jpg";
import watch02 from "../Images/watch-02.jpg";
import watch03 from "../Images/watch-03.jpg";
import watch04 from "../Images/watch-04.jpg";

import wireless01 from "../Images/wireless-01.png";
import wireless02 from "../Images/wireless-02.png";
import wireless03 from "../Images/wireless-03.png";
import wireless04 from "../Images/wireless-04.png";

import sofaSlide from "../Images/hero-img.png";
import watchSlide from "../Images/watch-07.png";

export const SliderData = [
  {
      id: 1,
      title: "Flat 40% Off For Your First Shopping",
      desc: "Dont Worry!You will get upto 30% discount second order onwards!",
      cover: sofaSlide,
  },
  {
      id: 2,
      title: "24*7 Customer Support",
      desc: "Contact Us - AnyTime AnyWhere",
      cover: phone08,
  },
  {
      id: 3,
      title: "Money Back Gurantee",
      desc: "If you doesn't like the product, We accept the return!",
      cover: wireless01,
  },

];

export const serviceData = [
  {
    icon: <ion-icon name="car"></ion-icon>,
    title: "Free Shipping",
    subtitle: "Shipping is free at your doorstep, anywhere in India.",
    bg: "#fdefe6",
  },
  {
    icon: <ion-icon name="card"></ion-icon>,
    title: "Safe Payment",
    subtitle: "Payment is safe thorough our RBI approved payment gateway.",
    bg: "#ceebe9",
  },

  {
    icon: <ion-icon name="headset"></ion-icon>,
    title: "Product Replacement Guarantee",
    subtitle: "If you get defective product, we will replace.",
    bg: "#d6e5fb",
  },
];

export const discoutProducts = [
  {
    id: "01",
    productName: "Stone and Beam Westview ",
    imgUrl: productImg01,
    category: "sofa",
    price: 192,
    discount:30,
    shortDesc:
      "Creative Sofa.... Description",
    description:
      "Folding Sofa....... Description are...",
    reviews: [
      {
        rating: 4.7,
        text: "Very Nice and durable.",
      },
    ],
    avgRating: 4.5,
  },
  {
    id: "02",
    productName: "Rivet Bigelow Modern ",
    imgUrl: productImg02,
    category: "sofa",
    price: 253,
    discount:20,
    shortDesc:
      "Sofa cum Bed",
    description:
      "Sofa cum Bed - Rivet Bigelow Modern",
    reviews: [
      {
        rating: 4.8,
        text: "Interesting Product",
      },
      {
        rating: 4.8,
        text: "very durable hand comfortable.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "08",
    productName: "Baltsar Chair",
    imgUrl: productImg08,
    category: "chair",
    price: 89,
    discount:15,
    shortDesc:
      "love",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "09",
    productName: "Helmar Chair",
    imgUrl: productImg09,
    category: "chair",
    price: 112,
    discount:35,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "12",
    productName: "Realme 8",
    imgUrl: phone03,
    category: "mobile",
    price: 599,
    discount:10,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },
  {
    id: "13",
    productName: "One Plus Nord",
    imgUrl: phone04,
    category: "mobile",
    price: 799,
    discount:5,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

]

export const products = [
  {
    id: "01",
    productName: "Stone and Beam Westview ",
    imgUrl: productImg01,
    category: "sofa",
    price: 193,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.7,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.5,
  },

  {
    id: "02",
    productName: "Rivet Bigelow Modern ",
    imgUrl: productImg02,
    category: "sofa",
    price: 253,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "03",
    productName: "Amazon Brand Modern Sofa",
    imgUrl: productImg03,
    category: "sofa",
    price: 173,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "2",
    productName: "Rivet Bigelow Modern ",
    imgUrl: productImg02,
    category: "sofa",
    price: 253,
    shortDesc:
      "i love uConsequuntur iure quas illo voluptates labore tempore!",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "04",
    productName: "Fllufy Sheep Sofa",
    imgUrl: productImg04,
    category: "sofa",
    price: 163,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "05",
    productName: "Faux Velvet Sofa",
    imgUrl: productImg05,
    category: "sofa",
    price: 163,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "06",
    productName: "Fllufy Sheep Sofa",
    imgUrl: productImg06,
    category: "sofa",
    price: 163,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "07",
    productName: "Sakarias Armchair",
    imgUrl: productImg07,
    category: "chair",
    price: 99,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "27",
    productName: "Modern Arm Sofa",
    imgUrl: productImg007,
    category: "sofa",
    price: 173,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "08",
    productName: "Baltsar Chair",
    imgUrl: productImg08,
    category: "chair",
    price: 89,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "09",
    productName: "Helmar Chair",
    imgUrl: productImg09,
    category: "chair",
    price: 112,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },

  {
    id: "10",
    productName: "Apple iPhone 12 Pro",
    imgUrl: phone01,
    category: "mobile",
    price: 799,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },
  {
    id: "25",
    productName: "Sakarias Armchair",
    imgUrl: productImg10,
    category: "chair",
    price: 99,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.6,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.7,
  },
  {
    id: "11",
    productName: "Apple iPhone 12 Max",
    imgUrl: phone02,
    category: "mobile",
    price: 799,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "12",
    productName: "Realme 8",
    imgUrl: phone03,
    category: "mobile",
    price: 599,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "13",
    productName: "One Plus Nord",
    imgUrl: phone04,
    category: "mobile",
    price: 799,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 2,
        text: "kjgkgggisicing elit.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "14",
    productName: "Apple iPhone 13 Pro",
    imgUrl: phone05,
    category: "mobile",
    price: 899,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "15",
    productName: "Samsung Galaxy S22",
    imgUrl: phone06,
    category: "mobile",
    price: 699,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "16",
    productName: "Rolex Watch",
    imgUrl: watch01,
    category: "watch",
    price: 299,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "17",
    productName: "Timex Easy Reader Watch",
    imgUrl: watch02,
    category: "watch",
    price: 299,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "18",
    productName: "Rolex Watch",
    imgUrl: watch03,
    category: "watch",
    price: 299,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "19",
    productName: "Apple Watch",
    imgUrl: watch04,
    category: "watch",
    price: 399,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "20",
    productName: "Beat Studio Wireless",
    imgUrl: wireless01,
    category: "wireless",
    price: 199,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

  {
    id: "21",
    productName: "Beat EP Headphones",
    imgUrl: wireless03,
    category: "wireless",
    price: 199,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },
  {
    id: "22",
    productName: "Black Headphones",
    imgUrl: wireless02,
    category: "wireless",
    price: 169,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },
  {
    id: "23",
    productName: "Bluetooth Headphones",
    imgUrl: wireless04,
    category: "wireless",
    price: 139,
    shortDesc:
      "Product Short Description Here",
    description:
      "Need to configure product description, its auto generated",
    reviews: [
      {
        rating: 4.8,
        text: "Very Nice Product, Durable! I love it.",
      },
      {
        rating: 4.9,
        text: "Very Nice Product, Durable! I love it.",
      },
    ],
    avgRating: 4.8,
  },

];
